...
if __name__ == '__main__':
    ...
    for _ in range(1):
        ...
        img_path = glob.glob(f'data/cifar10_64/cifar10-64/test/class{label}/img2330.png')
        img_mask_path = glob.glob(f'data/img2330_mask.png')
        print(f'image: {img_path[i_img]}')
        img = Image.open(img_path[i_img])
        mask = Image.open(img_path[i_img])
        # noise step
        noise_steps = 1000
        save_step = 5
        ...
        n = 1
        begin = 0
        exp_name = f'regenerate_guid_debug'  # 何をしたいか
